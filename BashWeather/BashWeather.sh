...
function getResponse {
   local URL="http://api.openweathermap.org/data/2.5/weather?"
    ...
    echo "Coordinates from IP location: LAT=$LAT, LON=$LON"
    local RESPONSEHOLDER=$(curl --connect-timeout $t -s "https://api.openweathermap.org/data/2.5/weather?lat=$LAT&lon=$LON&appid=$OPEN_WEATHER_API_KEY" 2>/dev/null)
...
function runWeather {
  ...
  local OPTIND
  while getopts ":c:l:u:s:t:k:eh" opt ; do
      case "$opt" in
  ...
  if [ "$ASSIGN_AGAIN" = "true" ] ; then
    ...
    local SUNSET=$(echo "$RESPONSEHOLDER" | grep -o -e '"sunset":[0-9]\{10\}' | tail -c 11)
      ...
      if [[ -n $WEATHERCODE ]] ; then
            ...
            elif [ "${WEATHERCODE:0:1}" -eq "7" ] ; then
              BASHWEATHERSYMBOL=$(get_symbol fog)
