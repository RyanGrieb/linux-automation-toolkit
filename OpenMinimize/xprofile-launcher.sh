...
# Launch Discord
"$OPEN_MINIMIZE" --name "Discord" --command "discord" --timeout 30 > "$LOG_DIR/discord.log" 2>&1 &
...
"$OPEN_MINIMIZE" --name "TickTick" --command "ticktick" --timeout 40 --recheck 5 > "$LOG_DIR/ticktick.log" 2>&1 &